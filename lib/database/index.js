...
import { getCache, setCache, invalidateCache } from '@lib/cache/cacheManager';
...
class ObjectData {
  ...
  clean = (
    ...
    this._cacheKey = null;
    this._cacheTTL = null;
    this._invalidatePatterns = null;
    this.hasDefaultWhere = !control.noDefaultWhere;
    return this;
  };

  /** Marks the next read query as cacheable
   * @param key Cache key
  ...
  /** Marks the next write operation to invalidate cache patterns
   * @param patterns Array of glob patterns to invalidate
   */
  invalidates = (patterns) => {
    this._invalidatePatterns = patterns;
    return this;
  };
  ...
  getAll = async () => {
    if (this._cacheKey) {
      const cached = await getCache(this._cacheKey);
      if (cached) {
        this.clean();
        return cached;
      }
    ...
    const result = this._count ? { count, data: response } : response;
    if (this._cacheKey) {
      await setCache(this._cacheKey, result, this._cacheTTL);
    }
    this.clean();
    return result;
  };
  ...
  getFirst = async () => {
    ...
    if (this._cacheKey) {
      await setCache(this._cacheKey, response, this._cacheTTL);
    }
    this.clean();
    return response;
  };
   ...
   */
  create = async (data) => {
    const patterns = this._invalidatePatterns;
